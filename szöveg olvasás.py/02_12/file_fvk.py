...
def stringSzetszedoToIntList(szoveg:str, separator=" ",elejerol_ennyi_nem_kell=0, vegerol_ennyi_nem_kell=0,adatseparator=""):
    egysor=szoveg.split(separator)
    if adatseparator is not None:
        egysor=egysor[
    szamok=strListToIntList(adatok)
    adatok=egysor[elejerol_ennyi_nem_kell:len(egysor)-vegerol_ennyi_nem_kell][0].split(adatseparator)
    return szamok
